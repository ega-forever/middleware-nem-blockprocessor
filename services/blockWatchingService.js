...
const bunyan = require('bunyan'),
  ...
  log = bunyan.createLogger({name: 'shared.services.blockWatchingService', level: config.logs.level});
...
class BlockWatchingService extends EventEmitter {
  ...
  async processBlock () {
    ...
    if (this.lastBlockHash !== null && this.currentHeight > 1) {
      let savedBlock = await models.blockModel.count({_id: lastBlock.prevBlockHash.data});
